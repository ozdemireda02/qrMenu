...
export function renderMenuItems(menuItems,menuList){
    ...
    .map((item) => `
      <a id="card" href="/detail.html?id=${item.id}" class="d-flex flex-column flex-md-row text-decoration-none gap-3">
              <img class="rounded shadow img-fluid" src="${item.img}" >
              <div>
                  <div class="d-flex justify-content-between">
                      <h5>${item.title}</h5>
                      <p class="text-success fw-bold">${(item.price * 27).toFixed(2) }₺</p>
                  </div>
  ...
  export function renderButtons(activeText){
    ...
    buttonData.forEach((btn) => {
        ...
        // eğerki eleman active ise bu classı ver
